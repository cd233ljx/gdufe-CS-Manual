<div class="mdx-hero">
  <h1>广财大学生活指南</h1>
  <p>
    由学生共建，<br>
    从录取通知书到熟门熟路。
  </p>

  <div class="mdx-hero__buttons">
    <a class="md-button md-button--primary" href="#courses-overview">
      课程总览
    </a>
    <a class="md-button" href="#campus-life">
      校园生活
    </a>
    <a class="md-button" href="campus/">
      校园知识库
    </a>
    <a class="md-button md-button--secondary" href="#contribute">
      参与共建
    </a>
  </div>

  <p style="margin-top: 1.5rem; opacity: .5; font-size: 0.82rem;">
    最近更新 · 2026.5.1 &nbsp;|&nbsp; 仍在建设中，欢迎 Star / Issue / PR
  </p>
</div>

## 这是什么

这里是 **广东财经大学学生共建的实用指南**——课程资源、校园生活、美食地图，一点一点攒起来的经验。

> 怎么用？
>
> - 左侧栏按「课程 / 校园生活」分类导航
> - 点击标题在首页内跳转
> - 已支持标题中文模糊搜索

---

## 快速入口

<div class="mdx-grid">

  <div class="mdx-card">
    <div class="mdx-card__header">
      <div class="mdx-card__icon">📚</div>
      <div class="mdx-card__title">课程资源</div>
    </div>
    <div class="mdx-card__content">
      按培养方案整理的课程索引，含经验贴与资料指路。先从这里了解你四年会学什么。
      <p>
        <a href="courses/c_index/">查看总课程索引 →</a>
      </p>
    </div>
  </div>

  <div class="mdx-card">
    <div class="mdx-card__header">
      <div class="mdx-card__icon">🍜</div>
      <div class="mdx-card__title">校园生活</div>
    </div>
    <div class="mdx-card__content">
      三水 / 广州校区的食堂、校外店、通勤路线、宿舍环境——帮你少踩坑。
      <p>
        <a href="#campus-life">跳转到生活指南 →</a>
      </p>
    </div>
  </div>

  <div class="mdx-card">
    <div class="mdx-card__header">
      <div class="mdx-card__icon">❓</div>
      <div class="mdx-card__title">常见问题</div>
    </div>
    <div class="mdx-card__content">
      学分怎么算？转专业 / 双学位？考试周怎么活？把大家问得最多的问题放在一起。
      <p>
        <a href="FaQ/faq/">看看常见疑问 →</a>
      </p>
    </div>
  </div>

  <div class="mdx-card">
    <div class="mdx-card__header">
      <div class="mdx-card__icon">🗂️</div>
      <div class="mdx-card__title">校园知识库</div>
    </div>
    <div class="mdx-card__content">
      选课考试、学籍制度、办事流程、毕业要求和生活服务的主题化索引。
      <p>
        <a href="campus/">进入知识库 →</a>
      </p>
    </div>
  </div>

  <div class="mdx-card">
    <div class="mdx-card__header">
      <div class="mdx-card__icon">🧰</div>
      <div class="mdx-card__title">学习工具箱</div>
    </div>
    <div class="mdx-card__content">
      常用网站、开发工具、效率插件、常见坑合集。写作业 / 做项目前先看一眼。
      <p>
        <a href="tools/tools/">打开工具箱 →</a>
      </p>
    </div>
  </div>

</div>

---

## <span id="courses-overview">课程资源总览</span>

> 当前以 **计算机科学与技术专业** 培养方案为主，其他专业后续逐步补齐。

- [总课程索引](courses/c_index.md)
- [基础课程导航](courses/basic/b_index.md)
- [专业核心课](courses/professional/p_index.md)
### [选修推荐](courses/elective.md)

如果你已经修过某门课，**欢迎补充：推荐教材 / 优秀作业 / 老师风格 / 复习建议等**。

---

## <span id="campus-life">校园生活指南</span>

#### 美食地图

#### [三水校区美食地图](life/food/sanshui/map.md)

#### [广州校区美食地图](life/food/guangzhou/map.md)

#### 其他

- [通勤](life/transport/transport.md)
- [宿舍](life/dormitory/dormitory.md)
- [音游地图及机厅评价](life/sbga/sbga.md)

- 以及还在施工中的更多内容……

> 想补充你的私藏路线 / 图书馆自习点心得？
> 直接来一发 👉 [投稿 & 贡献指南](contribute/contribute.md)

---

## <span id="contribute">一起让指南更好</span>

本指南因 **你的参与** 而有意义：

- 仓库地址 → [GitHub](https://github.com/cd233ljx/gdufe-CS-Manual)
- 发现错误或有建议 → [提交 Issue](https://github.com/cd233ljx/gdufe-CS-Manual/issues)
- 想补充课程笔记 / 生活经验 → [阅读贡献指南](contribute/contribute.md)
- 联系维护者 → <utopiacd@student.gdufe.edu.cn>

> **共建声明**：所有内容由学生自发整理，不代表学校官方观点，请理性参考。

## <span id="acknowledgments">致谢</span>
Inspiration from [Xeraphinite/gdufe-cs-manual](https://github.com/Xeraphinite/gdufe-cs-manual)

项目贡献者：
>
  Ostar_37

<script src="https://giscus.app/client.js"
        data-repo="cd233ljx/gdufe-CS-Manual"
        data-repo-id="R_kgDORJOQMA"
        data-category="General"
        data-category-id="DIC_kwDORJOQMM4C16ng"
        data-mapping="title"
        data-strict="0"
        data-reactions-enabled="1"
        data-emit-metadata="0"
        data-input-position="top"
        data-theme="preferred_color_scheme"
        data-lang="zh-CN"
        crossorigin="anonymous"
        async>
</script>
