...
css = """
        ...
        * {
        ...
        .approve { background-color: #69db7c; }
        ...
        .submission:nth-child(even) { transform: rotate(1deg); }
...
for f in files:
    ...
    # 1. Replace the entire <style>
    content = re.sub(r'<style>.*?</style>', f'<style>{css}</style>', content, flags=re.DOTALL)
    
    # 2. Replace the fonts link
    # The previous fonts were e.g., Noto+Sans+SC
    content = re.sub(r'<link href="https://fonts\.googleapis\.com.*? rel="stylesheet">', font_links, content, flags=re.DOTALL)
    
    # 3. Insert marquee after <body>
    if 'class="marquee"' not in content:
        body_tag = re.search(r'<body[^>]*>', content)
        if body_tag:
            content = content.replace(body_tag.group(0), f'{body_tag.group(0)}\n    <div class="marquee"><span>FOOD APP REDESIGN — BOLD NEO-BRUTALISM — NO MORE BLOBS — FLAVOR OVERLOAD — SUBMIT YOUR FAVORITES EXCLUSIVE</span></div>')

    with open(path, "w", encoding="utf-8") as file:
        file.write(content)

print(f"Successfully processed files: {', '.join(files)}")
